...
const getScoreInfo = (score) => {
  if (score >= 50) return { category: 'clean', label: 'Clean Gem', color: '#65A30D', bgColor: 'bg-[#65A30D]/10', textColor: 'text-[#65A30D]' };
  if (score >= 30) return { category: 'fair', label: 'Fair Choice', color: '#A67B5B', bgColor: 'bg-wood/10', textColor: 'text-wood-dark' };
  return { category: 'poor', label: 'Junk Vault', color: '#FF3B30', bgColor: 'bg-red-50', textColor: 'text-[#FF3B30]' };
};
...
const AlternativesGallery = ({ currentItem, recommendations, isSearching, onDismiss, onSelect }) => {
  return (
    ...
    >
      <div className="bg-[#F9F9F7] rounded-t-[3rem] p-8 pb-12 max-h-[85vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-8">
          ...
          <button onClick={onDismiss} className="p-2 bg-[#F7F5F0] rounded-full shadow-sm"><X className="w-5 h-5" /></button>
        </div>
        ...
        <div className="space-y-4">
          {isSearching && (
            <div className="py-20 text-center space-y-4">
              <motion.div animate={{ rotate: 360 }} transition={{ duration: 2, repeat: Infinity, ease: "linear" }}>
                <Cloud className="w-12 h-12 text-[#65A30D]/30 mx-auto" />
              </motion.div>
          ...
          {!isSearching && recommendations.map((gem, idx) => (
            <motion.div 
              ...
              className="bg-[#F7F5F0] p-4 rounded-3xl border border-stone-100 shadow-sm flex items-center gap-4 group active:scale-95 transition-all"
            >
              ...
              <div className="flex-1">
                <div className="flex justify-between items-start">
                  ...
                  <span className="text-xs font-black text-[#65A30D]">{gem.score}</span>
                </div>
                <div className="flex justify-between items-center mt-2">
                  ...
                  <div className="flex items-center gap-1 px-2 py-0.5 bg-stone-50 rounded-full border border-stone-100">
                    <MapPin className="w-2.5 h-2.5 text-[#65A30D]" />
                ...
                <button 
                  ...
                  className="mt-3 text-[10px] font-black text-[#65A30D] flex items-center gap-1 uppercase tracking-widest"
                >
        ...
        <div className="mt-10 p-6 bg-[#65A30D]/5 rounded-3xl border border-[#65A30D]/10 text-center">
          <p className="text-[11px] text-[#65A30D] font-medium italic">"Choosing better alternatives just 2 items a week can improve your pantry health by 30%."</p>
        </div>
      </div>
    </motion.div>
  );
...
const AuthScreen = ({ onGoogleSignIn, onContinueGuest }) => (
  <div className="absolute inset-0 bg-[#F9F9F7] z-[500] flex flex-col items-center justify-center px-10 text-center">
    <div className="w-64 h-64 flex items-center justify-center mb-12 overflow-hidden relative mt-12 rounded-full border border-stone-100 shadow-xl bg-[#F7F5F0] p-4">
    ...
    <div className="w-full space-y-4">
      <button 
        ...
        className="w-full bg-[#F7F5F0] border border-stone-100 py-4 rounded-[2rem] shadow-sm flex items-center justify-center gap-3 active:scale-95 transition-all group"
      >
      ...
      <button 
        ...
        className="w-full py-4 text-stone-300 text-[10px] font-bold uppercase tracking-widest hover:text-[#65A30D] transition-colors"
      >
        Continue as Guest
      </button>
    </div>
  ...
  return (
    ...
    >
      ...
      <div className="flex gap-4 items-start mb-6">
        {item.image && (
          <div className="w-20 h-20 rounded-2xl overflow-hidden bg-[#F7F5F0] border border-stone-100 flex-shrink-0 shadow-[0_8px_20px_rgba(0,0,0,0.08)] relative">
            <img 
              src={item.image} 
              alt={item.name} 
              ...
              {item.isNew ? (
                <div className="space-y-2">
                  <input 
                    ...
                    className="text-2xl font-bold mb-1 w-full border-b border-stone-200 outline-none focus:border-[#65A30D]"
                  ...
                  <input 
                    ...
                    className="text-xs font-bold text-stone-400 uppercase tracking-widest w-full border-b border-stone-100 outline-none focus:border-[#65A30D]"
      ...
      <div className="flex gap-2 mb-6 p-1 bg-stone-100 rounded-2xl">
        {!item.isNew && ['impact', 'ingredients', 'nutrition'].map(tab => (
          <button
            ...
            className={`flex-1 py-2 text-[10px] font-bold uppercase tracking-widest rounded-xl transition-all ${activeTab === tab ? 'bg-[#F7F5F0] text-stone-800 shadow-sm' : 'text-stone-400'}`}
          >
            {tab}
          </button>
        ))}
      ...
      <AnimatePresence mode="wait">
        {viewMode !== 'readonly' && (item.isNew || activeTab === 'impact') && (
          ...
          >
            {item.isNew ? (
              <div className="bg-[#65A30D]/5 border border-[#65A30D]/10 rounded-[2rem] p-8 text-center flex flex-col items-center">
                <motion.button 
                  whileTap={{ scale: 0.9 }}
                  onClick={handleCapture}
                  className="w-20 h-20 bg-[#F7F5F0] rounded-3xl flex items-center justify-center text-[#65A30D] shadow-[0_8px_20px_rgba(0,0,0,0.05)] mb-6 border border-[#65A30D]/10 relative overflow-hidden group"
                >
                  {capturedImage ? (
                  ...
                  <div className="absolute inset-0 bg-[#65A30D]/0 group-hover:bg-[#65A30D]/5 transition-colors" />
                </motion.button>
                ...
                <button 
                  onClick={handleCapture}
                  className="mt-6 px-5 py-2.5 bg-[#F7F5F0] border border-stone-100 rounded-full text-[10px] font-black text-[#65A30D] uppercase tracking-widest shadow-sm active:scale-95 transition-all"
                >
              ...
              <>
                {/* Meal Synergy Audit */}
                <div className="bg-[#65A30D]/5 border border-[#65A30D]/10 rounded-[2rem] p-5">
                  <div className="flex justify-between items-center mb-3">
                    <p className="text-[10px] font-medium text-stone-400 uppercase tracking-widest">Pantry Synergy</p>
                    <div className={`px-2 py-0.5 rounded-full text-[8px] font-bold uppercase bg-stone-100 text-stone-600`}>
                ...
                {/* Clean Points */}
                <div className="space-y-2">
                  <p className="text-[10px] text-stone-400 uppercase tracking-widest">Clean Points</p>
                  <div className="flex flex-wrap gap-2">
                    <span className="px-3 py-2 bg-stone-50 text-stone-600 rounded-xl text-[10px] font-medium flex items-center gap-1.5 border border-stone-100">
                ...
                {/* Bad Things */}
                <div className="space-y-2">
                  <p className="text-[10px] font-medium text-stone-400 uppercase tracking-widest">Watch Out</p>
                  <div className="flex flex-wrap gap-2">
                    {item.oils !== 'Clean' && (
                      <span className="px-3 py-2 bg-stone-50 text-stone-600 rounded-xl text-[10px] font-medium flex items-center gap-1.5 border border-stone-100">
          ...
          >
            ...
            <div className="max-h-72 overflow-y-auto space-y-2 pr-1 custom-scrollbar">
              {Array.isArray(resolvedIngredients) && resolvedIngredients.length > 0 ? (
                  resolvedIngredients.map((ing, idx) => {
                    ...
                    return (
                      <motion.div 
                        ...
                        key={idx} 
                        className="bg-[#F7F5F0] rounded-2xl border border-stone-100 px-4 py-3 shadow-sm"
                      >
                        <div className="flex items-center justify-between gap-3">
                          <p className="text-sm font-bold text-stone-800 capitalize leading-tight flex-1">{name}</p>
                          <div className="flex items-center gap-2 flex-shrink-0">
                            {isNonVegan && (
                              <span className="text-[8px] font-black text-[#FF3B30] uppercase tracking-wider bg-[#FF3B30]/10 px-1.5 py-0.5 rounded-full">Non-Vegan</span>
                            )}
                            {pct !== null && (
                        ...
                        {pct !== null && (
                          <div className="mt-2 h-1 bg-stone-100 rounded-full overflow-hidden">
                            <motion.div
                              ...
                              className={`h-full rounded-full ${pct > 40 ? 'bg-[#65A30D]' : pct > 15 ? 'bg-wood' : 'bg-stone-300'}`}
                            />
                          </div>
                        )}
                      </motion.div>
                  ...
                  <div className="bg-[#F7F5F0] p-5 rounded-2xl border border-stone-100 shadow-sm">
                  ...
                  <div className="flex items-center justify-center gap-3 py-10 text-stone-400">
                    <div className="w-5 h-5 border-2 border-[#65A30D]/30 border-t-sage rounded-full animate-spin" />
                  ...
                  <div className="flex flex-col items-center justify-center bg-stone-50/50 p-10 rounded-[3rem] border border-stone-100 border-dashed">
                    <div className="w-16 h-16 bg-[#F7F5F0] rounded-3xl flex items-center justify-center text-[#65A30D]/30 shadow-sm mb-4 border border-stone-50">
                    ...
                    <div className="mt-6 px-4 py-2 bg-[#F7F5F0] border border-stone-100 rounded-full text-[9px] font-black text-[#65A30D] uppercase tracking-widest shadow-sm">
            ...
            {[
              ...
              >
                <div className="flex items-center gap-4 p-4 group">
                  <div className={`p-2 rounded-xl text-stone-400 group-hover:bg-[#65A30D]/10 group-hover:text-[#65A30D] transition-colors ${row.isAdditives ? 'bg-[#65A30D]/5' : 'bg-stone-50'}`}>
                    {row.icon}
                  </div>
                  <div className="flex-1">
                    <div className="flex justify-between items-center">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className={`w-2.5 h-2.5 rounded-full ${row.status === 'good' ? 'bg-[#65A30D] shadow-[0_0_8px_rgba(93,109,63,0.4)]' : row.status === 'bad' ? 'bg-[#FF3B30] shadow-[0_0_8px_rgba(210,125,86,0.4)]' : 'bg-yellow-400 shadow-[0_0_8px_rgba(250,204,21,0.4)]'}`} />
                      </div>
                    </div>
                    {row.isAdditives ? (
                    ...
                    <div className="px-4 pb-4 space-y-3">
                      {resolvedAdditives.map((tag, i) => {
                        ...
                        const riskColors = {
                          ...
                          safe: { bg: 'bg-[#65A30D]/5', text: 'text-[#65A30D]', border: 'border-[#65A30D]/10', badge: 'bg-[#65A30D]' },
                        };
  ...
  return (
    ...
    >
      <div className="flex items-center gap-4 mb-8">
        <button onClick={onBack} className="p-2 bg-[#F7F5F0] rounded-full shadow-sm transition-transform active:scale-90"><ChevronRight className="w-5 h-5 rotate-180" /></button>
        <h2 className="text-2xl font-bold">{title}</h2>
      </div>
      
      <div className="space-y-4 overflow-y-auto h-[calc(100%-100px)] custom-scrollbar">
        {filteredItems.length > 0 ? filteredItems.map((item, idx) => (
          <div key={idx} className="bg-[#F7F5F0] p-4 rounded-[2rem] border border-stone-100 shadow-sm flex items-center gap-4">
            ...
            <div className="flex-1">
              ...
              <div className="flex justify-between items-center mt-2">
                ...
                <div className="text-right">
                  <p className="text-[10px] font-medium text-stone-400">{item.expiry || 'Expiry date'}</p>
                  <p className="text-[8px] text-stone-300 font-medium uppercase">{item.expiryDate}</p>
                </div>
              </div>
            </div>
          </div>
...
const Dashboard = ({ stats, user, historyItems, onSelectCategory, onShowMarket, setShowAuth, setUser, setCurrentScreen }) => {
  ...
  return (
    <motion.div 
      initial={{ opacity: 0 }} 
      animate={{ opacity: 1 }} 
      className="bg-[#F9F9F7] min-h-screen pb-12"
    >

      {/* Sleek Single-Line Header - Pure White */}
      <div className="px-6 py-2 backdrop-blur-md flex justify-between items-center border-b border-slate-100" style={{ paddingTop: 'calc(env(safe-area-inset-top) + 0.25rem)', backgroundColor: '#F8FAFC' }}>
        {/* Left: Profile & Greeting */}
        <div className="flex items-center gap-3 flex-1">
          <div className={`w-10 h-10 rounded-xl overflow-hidden bg-[#F7F5F0] border-2 border-white shadow-md transition-all duration-700 ${!user?.isAnonymous ? 'ring-2 ring-sage/10' : ''}`}>
        ...
        <div className="flex-[4] flex justify-center px-4 overflow-hidden">
          <img 
            src={logoOfficial} 
            alt="PantryBloom" 
            className="h-16 w-full object-contain scale-110" 
            style={{ 
        ...
        <div className="flex items-center gap-2 flex-1 justify-end">
          <button 
            ...
            className="p-2 bg-[#F7F5F0] text-wood-dark rounded-lg hover:bg-stone-50 transition-all shadow-sm"
            title="Local Market"
          >
          ...
          {user && !user.isAnonymous && (
            <button 
              ...
              className="p-2 bg-[#F7F5F0] text-stone-400 rounded-lg hover:bg-stone-50 transition-all shadow-sm"
              title="Log Out"
            >
      ...
      <div className="flex flex-col items-center pt-2 pb-6 bg-[#F9F9F7]">
        ...
        <div className="text-center mt-2">
          ...
          <div className={`mt-4 px-6 py-2 rounded-full font-bold text-sm inline-block shadow-sm ${
            isEmpty ? 'bg-[#65A30D]/10 text-[#65A30D]' : getScoreInfo(healthScore).bgColor + ' ' + getScoreInfo(healthScore).textColor
          }`}>
      ...
      {/* Pantry Overview (Tighter Boutique Style) */}
      <div className="px-5 mt-3">
        <div className="bg-[#F7F5F0] p-4 rounded-[2rem] border border-stone-100/50 shadow-sm">
          <h3 className="text-sm font-bold text-stone-800 mb-4 px-1">Pantry Overview</h3>
          <div className="grid grid-cols-3 gap-2">
            {/* Good Items Card */}
            <div className="bg-white p-3 rounded-2xl flex flex-col items-center text-center border border-stone-50 shadow-sm">
              <div className="w-8 h-8 rounded-full bg-[#65A30D] flex items-center justify-center text-white mb-2 shadow-sm border-2 border-white">
                <CheckCircle2 className="w-4 h-4" />
              </div>
              <p className="text-xl font-bold text-stone-800 leading-none mb-1">{stats.cleanCount}</p>
              <p className="text-[10px] text-[#65A30D] font-bold">Good Items</p>
            </div>
            
            {/* Bad / Expired Card */}
            <div className="bg-white p-3 rounded-2xl flex flex-col items-center text-center border border-stone-50 shadow-sm">
              <div className="w-8 h-8 rounded-full bg-[#FF3B30] flex items-center justify-center text-white mb-2 shadow-sm border-2 border-white">
                <XCircle className="w-4 h-4" />
              </div>
              <p className="text-xl font-bold text-stone-800 leading-none mb-1">{stats.junkyCount + stats.expiredCount}</p>
              <p className="text-[10px] text-[#FF3B30] font-bold">Bad / Expired</p>
            </div>
            
            {/* Expiring Soon Card */}
            <div className="bg-white p-3 rounded-2xl flex flex-col items-center text-center border border-stone-50 shadow-sm">
              <div className="w-8 h-8 rounded-full bg-orange-500 flex items-center justify-center text-white mb-2 shadow-sm border-2 border-white">
                <Bell className="w-4 h-4" />
              </div>
              <p className="text-xl font-bold text-stone-800 leading-none mb-1">{stats.expiringCount}</p>
              <p className="text-[10px] text-stone-600 font-bold">Expiring Soon</p>
            </div>
          </div>
        </div>
      </div>

      {/* Expiring Soon Integrated List Panel */}
      <div className="px-5 mt-4">
        <div className="bg-[#F7F5F0] rounded-[2rem] border border-stone-100/50 shadow-sm overflow-hidden">
          <div className="p-5 pb-2 flex items-center justify-between">
            <h3 className="text-orange-600 font-bold text-base">Expiring Soon</h3>
            <button onClick={() => onSelectCategory('expiring', 'Expiring Soon')} className="text-[#65A30D] font-bold text-xs">View all</button>
          </div>
            ...
            {stats.expiringItems.length > 0 ? (
              stats.expiringItems.slice(0, 3).map((item, idx) => (
                <button 
                  ...
                  className="w-full p-4 bg-[#F7F5F0] flex items-center gap-4 active:bg-stone-50 transition-colors text-left"
                >
                  <div className="w-12 h-12 rounded-xl bg-white overflow-hidden flex-shrink-0 border border-stone-50 shadow-sm">
                    {item.image ? (
                      <img src={item.image} alt={item.name} className="w-full h-full object-contain p-1" />
                    ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-orange-500 font-bold">Expires in {Math.ceil((new Date(item.expiryDate) - new Date()) / (1000 * 60 * 60 * 24))} days</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="px-3 py-1 bg-orange-50 text-orange-600 text-[10px] font-bold rounded-lg border border-orange-100/50">
              ...
              <div className="p-8 text-center text-stone-400 text-[10px] font-bold uppercase tracking-widest">
      ...
      {/* Recently Added Integrated List Panel */}
      <div className="px-5 mt-4 pb-12">
        <div className="bg-[#F7F5F0] rounded-[2rem] border border-stone-100/50 shadow-sm overflow-hidden">
          <div className="p-5 pb-2 flex items-center justify-between">
            <h3 className="text-stone-800 font-bold text-base">Recently Added</h3>
            <button onClick={() => setCurrentScreen('pantry')} className="text-[#65A30D] font-bold text-xs">View all</button>
          </div>
          
          <div className="divide-y divide-stone-100/80">
            {historyItems.slice(0, 1).map((item, idx) => (
              <button 
                key={idx}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-bold text-stone-800 text-sm truncate">{item.name}</p>
                  <p className="text-[10px] text-[#65A30D] font-bold">Added today</p>
                </div>
                <ChevronRight className="w-4 h-4 text-stone-300" />
              </button>
            ))}
...
const HistoryScreen = ({ items, onDelete, onItemClick }) => (
  ...
  >
    <div className="flex justify-between items-center mb-8">
      ...
      <div className="px-3 py-1 bg-[#65A30D]/10 rounded-full">
        <p className="text-[10px] text-[#65A30D] font-bold uppercase tracking-widest">{items.length} Logs</p>
      </div>
    </div>
      ...
      {items.length === 0 ? (
        ...
        items.map((item, idx) => (
          <motion.div 
            ...
            className="bg-[#F7F5F0] p-4 rounded-[2rem] border border-stone-100 shadow-sm flex items-center gap-4 group relative overflow-hidden cursor-pointer active:scale-95 transition-all"
          >
            ...
            <button 
              ...
              className="p-2 text-stone-200 hover:text-[#FF3B30] transition-colors"
            >
  ...
  >
    ...
    <div className="cozy-card mb-8 p-8 flex flex-col items-center">
      ...
      <div className="flex gap-6 mt-4">
        <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-[#65A30D]" /><span className="text-[10px] font-bold">Good</span></div>
        ...
        <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-[#FF3B30]" /><span className="text-[10px] font-bold">Poor</span></div>
      </div>
    </div>
    ...
    <div className="space-y-4">
      {[
        { id: 'week', label: 'This Week', count: stats.weekCount, color: 'bg-[#FF3B30]' },
        ...
        return (
          <button 
            ...
            className="w-full bg-[#F7F5F0] p-5 rounded-[2rem] border border-stone-100 text-left active:scale-95 transition-all"
          >
            <div className="flex justify-between items-center mb-3">
  ...
  return (
    <div className={`relative w-full h-full overflow-hidden ${isNative ? 'bg-transparent' : 'bg-black'}`}>
      ...
      <div className="absolute top-10 left-6 right-6 flex justify-between items-center text-white z-20">
        ...
        <button className="p-3 bg-[#F7F5F0]/20 rounded-full backdrop-blur-md"><Settings className="w-5 h-5" /></button>
      </div>
      ...
      <div className="absolute bottom-12 left-0 right-0 flex flex-col items-center gap-6 px-8 z-20">
        ...
        <button 
          ...
          className="w-16 h-16 bg-[#F7F5F0] rounded-full flex items-center justify-center border-4 border-white/40 active:scale-95 transition-all shadow-2xl"
        >
          ...
          >
            <div className="w-full space-y-6">
              ...
              <input 
                ...
                className="w-full bg-[#F7F5F0]/5 border border-white/10 rounded-2xl py-4 px-6 text-white text-center text-xl outline-none focus:border-[#65A30D] transition-colors"
              />
              <div className="flex gap-3">
                <button onClick={() => setShowManual(false)} className="flex-1 py-4 bg-[#F7F5F0]/5 text-white/60 rounded-2xl font-bold text-xs uppercase tracking-widest">Cancel</button>
                <button 
                  onClick={() => { 
                    if(manualCode) {
                      onScan(manualCode); 
                  ...
                  className="flex-1 py-4 bg-[#65A30D] text-white rounded-2xl font-bold text-xs uppercase tracking-widest"
                >
                  Audit Product
                </button>
              </div>
  ...
  return (
    ...
    >
      <div className="p-8 flex items-center gap-4">
        <button onClick={onBack} className="p-2 bg-[#F7F5F0] rounded-full shadow-sm"><ChevronRight className="w-5 h-5 rotate-180" /></button>
        ...
        <div className="w-full h-56 bg-[#f4f1ea] rounded-[3rem] relative overflow-hidden shadow-inner border-4 border-white">
           ...
           <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex items-center justify-center pointer-events-none">
             <motion.div 
               ...
               className="absolute w-12 h-12 bg-[#65A30D] rounded-full" 
             />
             <div className="relative w-4 h-4 bg-[#65A30D] rounded-full border-2 border-white shadow-lg" />
           </div>
           ...
           {markets.map((m, i) => (
             ...
             >
               <div className="w-3 h-3 bg-[#FF3B30] rounded-full border-2 border-white shadow-sm" />
             </motion.div>
           ))}
           
           <div className="absolute bottom-4 left-6 px-3 py-1 bg-[#F7F5F0]/80 backdrop-blur-md rounded-full text-[8px] font-bold text-stone-400 uppercase tracking-widest shadow-sm">
             Wyndham Vale, VIC
           </div>
        </div>

        {markets.map((market, idx) => (
          <div key={idx} className="bg-[#F7F5F0] p-5 rounded-[2rem] border border-stone-100 shadow-sm flex justify-between items-center group">
            <div>
              ...
              <div className="flex gap-2">
                <span className="px-2 py-0.5 bg-[#65A30D]/10 text-[#65A30D] text-[8px] font-bold rounded-full">{market.day}</span>
            ...
            <div className="text-right">
              ...
              <button 
                ...
                className="text-[10px] font-bold text-[#65A30D] underline mt-1"
              >
                Get Directions
              </button>
            </div>
...
const Pantry = ({ items, onItemClick }) => {
  ...
  return (
    <motion.div 
      initial={{ opacity: 0 }} 
      animate={{ opacity: 1 }} 
      className="px-6 py-8 pb-32 bg-[#F9F9F7]"
    >
        ...
        <div className="flex items-center gap-2">
          <div className="bg-stone-100 p-1 rounded-xl flex gap-1">
            <button 
              ...
              className={`p-1.5 rounded-lg transition-all ${viewMode === 'grid' ? 'bg-[#F7F5F0] shadow-sm text-[#65A30D]' : 'text-stone-400'}`}
            >
              <LayoutGrid className="w-4 h-4" />
            </button>
            <button 
              onClick={() => setViewMode('list')}
              className={`p-1.5 rounded-lg transition-all ${viewMode === 'list' ? 'bg-[#F7F5F0] shadow-sm text-[#65A30D]' : 'text-stone-400'}`}
            >
          ...
          <button 
            ...
            className="w-12 h-12 bg-[#65A30D] text-white rounded-2xl shadow-lg flex items-center justify-center active:scale-90 transition-transform"
          >
      ...
      <div className="flex gap-2 mb-6 p-1 bg-stone-100 rounded-2xl">
        <button
          ...
          className={`flex-1 py-3 text-[10px] font-bold uppercase tracking-widest rounded-xl transition-all flex items-center justify-center gap-2 ${activeTab === 'clean' ? 'bg-[#F7F5F0] text-[#65A30D] shadow-sm' : 'text-stone-400'}`}
        >
        ...
        <button
          ...
          className={`flex-1 py-3 text-[10px] font-bold uppercase tracking-widest rounded-xl transition-all flex items-center justify-center gap-2 ${activeTab === 'junk' ? 'bg-[#F7F5F0] text-[#FF3B30] shadow-sm' : 'text-stone-400'}`}
        >
      ...
      <div className="relative mb-8">
        <input 
          ...
          className="w-full bg-[#F7F5F0] border border-stone-100 rounded-2xl py-3.5 px-6 text-sm shadow-sm outline-none focus:border-[#65A30D] transition-all pl-12"
        />
        <div className="absolute left-4 top-1/2 -translate-y-1/2">
           {activeTab === 'clean' ? <Leaf className="w-4 h-4 text-[#65A30D] opacity-30" /> : <AlertCircle className="w-4 h-4 text-[#FF3B30] opacity-30" />}
        </div>
      </div>
...
const PantryListItem = ({ item, onClick }) => {
  ...
  return (
    <motion.div 
      onClick={onClick}
      className="bg-[#F7F5F0] p-3 rounded-2xl border border-stone-100 shadow-sm flex items-center gap-4 cursor-pointer active:scale-[0.98] transition-all"
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="mt-2 flex items-center gap-3">
          <div className="flex-1 h-1 bg-stone-50 rounded-full overflow-hidden">
            <div className="h-full transition-all duration-500" style={{ width: `${item.score}%`, backgroundColor: getScoreInfo(item.score).color }} />
          </div>
          <span className={`text-[8px] font-medium uppercase text-stone-400`}>
...
const PantryCard = ({ item, onClick }) => {
  ...
  return (
    <motion.div 
      whileHover={{ y: -5 }}
      onClick={onClick}
      className="bg-[#F7F5F0] p-3 rounded-[2rem] border border-stone-100 shadow-sm flex flex-col items-center text-center cursor-pointer active:scale-95 transition-all relative group"
    >
      {(isExpiring || isExpired) && (
        <div className={`absolute top-3 right-3 w-2 h-2 ${isExpired ? 'bg-[#FF3B30]' : 'bg-orange-500'} rounded-full animate-pulse shadow-lg`} />
      )}
...
const App = () => {
  ...
  const stats = React.useMemo(() => {
    ...
    const expiringMonth = pantryItems.filter(i => {
    ...
    return {
      ...
      junkyCount: poor.length + fair.length,
      ...
      fairItems: fair,
      distData: [
        { name: 'Good', value: clean.length, color: '#65A30D' },
        { name: 'Fair', value: fair.length, color: '#A67B5B' },
        { name: 'Poor', value: poor.length, color: '#FF3B30' }
      ]
    };
  }, [pantryItems]);
  ...
  return (
    <div className="w-full h-[100dvh] bg-zinc-200 sm:flex sm:items-center sm:justify-center sm:p-4 overflow-hidden">
      <div className="phone-container">
        ...
        <AnimatePresence>
          {isAuthLoading ? (
            <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 z-[600] bg-[#F9F9F7] flex flex-col items-center justify-center">
              <div className="w-16 h-16 border-4 border-[#65A30D]/20 border-t-sage rounded-full animate-spin mb-4" />
              <p className="text-[10px] font-black text-[#65A30D] uppercase tracking-[0.3em]">Boutique Loading...</p>
            </motion.div>
          ) : (!user || showAuth) ? (
        ...
        {/* Screen Content */}
        <div ref={mainScrollRef} className="w-full h-full pb-16 overflow-y-auto custom-scrollbar relative">
          <AnimatePresence mode="wait">
        ...
        {/* Tab Bar */}
        <nav className="absolute bottom-0 left-0 right-0 h-20 bg-[#F7F5F0] border-t border-slate-100 flex items-center justify-around z-50 px-4">
            <button 
              onClick={() => setCurrentScreen('dashboard')}
              className={`flex flex-col items-center gap-1.5 transition-all relative group ${currentScreen === 'dashboard' ? 'text-[#65A30D]' : 'text-[#65A30D]/40'}`}
            >
              {currentScreen === 'dashboard' && <motion.div layoutId="navGem" className="absolute -inset-2 bg-[#65A30D]/10 rounded-2xl -z-10" />}
              <Home className={`w-5 h-5 ${currentScreen === 'dashboard' ? 'stroke-[2.5px]' : 'stroke-[1.5px]'}`} />
              <span className="text-[7px] font-black uppercase tracking-[0.2em]">Home</span>
            </button>

            <button 
              onClick={() => setCurrentScreen('history')}
              className={`flex flex-col items-center gap-1.5 transition-all relative group ${currentScreen === 'history' ? 'text-wood' : 'text-wood/40'}`}
            >
              {currentScreen === 'history' && <motion.div layoutId="navGem" className="absolute -inset-2 bg-wood/10 rounded-2xl -z-10" />}
              <History className={`w-5 h-5 ${currentScreen === 'history' ? 'stroke-[2.5px]' : 'stroke-[1.5px]'}`} />
              <span className="text-[7px] font-black uppercase tracking-[0.2em]">History</span>
            </button>

            <button 
              onClick={() => setCurrentScreen('scanner')}
              className={`flex flex-col items-center gap-1.5 transition-all relative group ${currentScreen === 'scanner' ? 'text-slate-600' : 'text-slate-600/40'}`}
            >
              {currentScreen === 'scanner' && <motion.div layoutId="navGem" className="absolute -inset-2 bg-slate-600/10 rounded-2xl -z-10" />}
              <div className="w-10 h-10 bg-[#F7F5F0] rounded-full flex items-center justify-center shadow-lg -translate-y-4 border-4 border-[#F8FAFC]">
                 <Barcode className={`w-6 h-6 ${currentScreen === 'scanner' ? 'text-slate-600' : 'text-slate-600'}`} />
              </div>
              <span className="text-[7px] font-black uppercase tracking-[0.2em] -mt-4">Scan</span>
            </button>

            <button 
              onClick={() => setCurrentScreen('pantry')}
              className={`flex flex-col items-center gap-1.5 transition-all relative group ${currentScreen === 'pantry' ? 'text-sage' : 'text-sage/40'}`}
            >
              {currentScreen === 'pantry' && <motion.div layoutId="navGem" className="absolute -inset-2 bg-sage/10 rounded-2xl -z-10" />}
              <Refrigerator className={`w-5 h-5 ${currentScreen === 'pantry' ? 'stroke-[2.5px]' : 'stroke-[1.5px]'}`} />
              <span className="text-[7px] font-black uppercase tracking-[0.2em]">Pantry</span>
            </button>

            <button 
              onClick={() => setCurrentScreen('stats')}
              className={`flex flex-col items-center gap-1.5 transition-all relative group ${currentScreen === 'stats' ? 'text-blue-500' : 'text-blue-500/40'}`}
            >
              {currentScreen === 'stats' && <motion.div layoutId="navGem" className="absolute -inset-2 bg-blue-500/10 rounded-2xl -z-10" />}
              <BarChart3 className={`w-5 h-5 ${currentScreen === 'stats' ? 'stroke-[2.5px]' : 'stroke-[1.5px]'}`} />
              <span className="text-[7px] font-black uppercase tracking-[0.2em]">Stats</span>
            </button>
        </nav>
